...
const App = () => {
  ...
  useEffect(() => {
    IcebreakersService.getAllIceBreakers().then((iceBreakers) =>
  ...
  const handleAddDatePlan = async (newDatePlanData) => {
    const newDatePlan = await datePlanService.create(
      newDatePlanData,
      user.profile
    );
  ...
  const handleEditDatePlan = (updatedDatePlan) => {
    datePlanService.update(updatedDatePlan).then((updatedDatePlan) => {
  ...
  const handleDeleteDatePlan = (id) => {
    datePlanService.deleteOne(id).then((deleteDatePlan) => {
      setDatePlans(
        datePlans.filter((datePlan) => datePlan._id !== deleteDatePlan._id)
      );
      navigate(`/profiles/${profiles[proIdx]._id}`);
    });
  };
  ...
  const handleDeleteIceBreaker = (id) => {
    ...
    const updatedIceBreakers = iceBreakers.filter(
      (iceBreaker) => iceBreaker._id !== id
    );
  ...
  const handleEditProfile = async (updatedProfile) => {
    profileService
      .update(updatedProfile, user.profile)
      .then((updatedProfile) => {
          ...
          element={
            user ? (
              <Icebreakers
                ...
                handleDeleteIceBreaker={handleDeleteIceBreaker}
              />
            ) : (
              <Navigate to="/login" />
            )
        ...
        <Route
          ...
          element={
            user ? (
              <EditDatePlan
                user={user}
                handleEditDatePlan={handleEditDatePlan}
              />
            ) : (
              <Navigate to="/signin" />
            )
        ...
        <Route
          ...
          element={
            user ? (
              <AddIceBreaker
                user={user}
                handleAddIceBreaker={handleAddIceBreaker}
              />
            ) : (
              <Navigate to="/signin" />
            )
